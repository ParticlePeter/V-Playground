...
layout(binding = 3) uniform sampler2D heightmap_linear;
...
vec4 sd_heightmap(vec3 ro, vec3 rd) {//, inout float depth, inout vec3 normal) {
    ...
    // 2. sample hight from texture corresponding to p and q uv values to get line segment
    q.y = HM_Height_Factor * textureLod(heightmap_linear, q.xz, level).r;
    r.y = HM_Height_Factor * textureLod(heightmap_linear, r.xz, level).r;
	*/
